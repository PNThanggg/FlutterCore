...
class EnumToString {
  ...
  static T? fromString<T>(List<T> enumValues, String value, {bool camelCase = false}) {
    try {
      return enumValues.singleWhere((enumItem) =>
          EnumToString.convertToString(enumItem, camelCase: camelCase).toLowerCase() == value.toLowerCase());
  ...
  static List<String> toList<T>(List<T> enumValues, {bool camelCase = false}) {
    final enumList = enumValues
        .map((t) => !camelCase ? EnumToString.convertToString(t) : EnumToString.convertToString(t, camelCase: true))
        .toList();
